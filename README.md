...
为了让博客正常工作并支持 HTTPS，您需要在反向代理软件（如 Nginx, 宝塔面板, Nginx Proxy Manager）中配置**3条转发规则**：

| 路径 (Path) | 目标 URL (Target) | 说明 |
|------------|-------------------|------|
| `/` | `http://127.0.0.1:5173` | 前端页面 |
| `/api` | `http://127.0.0.1:3001` | 后端数据接口 |
| `/socket.io` | `http://127.0.0.1:3001` | 实时监控 (需开启 WebSocket 支持) |

> **注意**：如果不配置 `/api` 和 `/socket.io` 的转发，页面将无法加载数据或显示监控信息。
